...
const getDatabaseKey = async () => {
  const { data } = await useFetch(`/api/db/${state.user!.ref}`).json();
...
<template>
  <main class="col center fade-in gap-4 p-20 text-center">
    ...
    <div v-if="state.database" class="gap-4">
      <p class="my-4">
        <strong>Database ID: </strong>: {{ state.database.database }}
      </p>
      <p class="my-4">
        <strong>Database Secret: </strong
        >{{ show ? state.database.secret : "*******" }}
      </p>
      <p class="row center">
        <Icon
          :icon="show ? 'mdi-eye-off' : 'mdi-eye'"
          @click="show = !show"
          class="cp scale rf m-4 sh x2 p-2"
        />
        <Icon
          icon="mdi-clipboard"
    ...
    <div v-else>
      <button class="btn-get" @click="getDatabaseKey()">
        Get Database Key
      </button>
    </div>
  </main>
